...
const LastData = GetConnect({name: 'LastData' , deamon : initional}).define(TABLENAME, {
            id: {
                            type: DataTypes.INTEGER,
                                        autoIncrement: true,
                primaryKey: true
...
const Update = async (data) => {
    ...
    await LastData.update(data, { where: { id: data.id } })
...
const GetAllFilterCount = async (filter) => {
    ...
    return LastData.count({ where:filter,...filter})
}
