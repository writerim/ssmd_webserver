...
const Rule = GetConnect({name: 'Rule' , deamon : initional}).define(TABLENAME, {
            id: {
                            type: DataTypes.INTEGER,
                                        autoIncrement: true,
                primaryKey: true
...
const Update = async (data) => {
    ...
    await Rule.update(data, { where: { id: data.id } })
...
const GetAllFilterCount = async (filter) => {
    ...
    return Rule.count({ where:filter,...filter})
}
...
RULE_ERROR_NOT_FOUND: ERROR_NOT_FOUND,
    ...
    RuleGetAllSearch: GetAllSearch,
    RuleGetAllSearchCount: GetAllSearchCount,
    }