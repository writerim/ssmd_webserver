...
const SettingsEnterprise = GetConnect({name: 'SettingsEnterprise' , deamon : initional}).define(TABLENAME, {
            id: {
                            type: DataTypes.INTEGER,
                                        autoIncrement: true,
                primaryKey: true
...
const Update = async (data) => {
    ...
    await SettingsEnterprise.update(data, { where: { id: data.id } })
...
const GetAllFilterCount = async (filter) => {
    ...
    return SettingsEnterprise.count({ where:filter,...filter})
}
...
SETTINGSENTERPRISE_ERROR_NOT_FOUND: ERROR_NOT_FOUND,
    ...
    SettingsEnterpriseGetAllSearch: GetAllSearch,
    SettingsEnterpriseGetAllSearchCount: GetAllSearchCount,
    }