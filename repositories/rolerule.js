...
const initional = ()=>{
            ...
            GetConnect().getQueryInterface().addColumn(TABLENAME + 's', 'role_id', { 
                    type: DataTypes.INTEGER,
                }, {  mustExist: false }).catch(()=>{});
            GetConnect().getQueryInterface().addColumn(TABLENAME + 's', 'rule_id', { 
                    type: DataTypes.INTEGER,
                }, {  mustExist: false }).catch(()=>{});
...
const RoleRule = GetConnect({name: 'RoleRule' , deamon : initional}).define(TABLENAME, {
            id: {
                            type: DataTypes.INTEGER,
                                        autoIncrement: true,
                primaryKey: true
...
const Update = async (data) => {
    ...
    await RoleRule.update(data, { where: { id: data.id } })
...
const GetAllFilterCount = async (filter) => {
    ...
    return RoleRule.count({ where:filter,...filter})
}
