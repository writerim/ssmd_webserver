...
const User = GetConnect({name: 'User' , deamon : initional}).define(TABLENAME, {
            id: {
                            type: DataTypes.INTEGER,
                                        autoIncrement: true,
                primaryKey: true
...
const GetAllFilterCount = async (filter) => {
    ...
    return User.count({ where:filter,...filter})
}
...
USER_ERROR_NOT_FOUND: ERROR_NOT_FOUND,
    ...
    UserGetAllSearch: GetAllSearch,
    UserGetAllSearchCount: GetAllSearchCount,
