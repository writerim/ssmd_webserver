...
const { 
    ...
    UserGetAllFilter,
    UserGetAllIncludesObject,
    UserGetAllIncludesObjectCount,
        UserRecalcTree ,
...
module.exports = {
    ...
    async GetAllUser(params, flags , user_ctx) {
        if (!user_ctx || typeof user_ctx != 'object' || !(user_ctx instanceof UserCtx)) {
            throw new Error(NOT_FOUND_CONTEXT)
        }
        return UserGetAll(params, flags).then(res=>{
            let output = []
            res.forEach(r => {
    ...
    async GetAllByFilterUser(filter, params , flags, user_ctx) {
        if (!user_ctx || typeof user_ctx != 'object' || !(user_ctx instanceof UserCtx)) {
            throw new Error(NOT_FOUND_CONTEXT)
        }
        return UserGetAllFilter(filter, params, flags).then(res=>{
            let output = []
            res.forEach(r => {
                output.push(new User(r.dataValues))
            })
    ...
    async GetAllSearchUser(text, params , flags, user_ctx) {
        if (!user_ctx || typeof user_ctx != 'object' || !(user_ctx instanceof UserCtx)) {
            throw new Error(NOT_FOUND_CONTEXT)
        }
        return UserGetAllSearch(text, params, flags).then(res=>{
            let output = []
            res.forEach(r => {
                output.push(new User(r.dataValues))
            })
