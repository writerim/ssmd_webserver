...
describe("use_cases: LastData", function () {
        ...
        it('func LastData Add. Проверка на контекст', async function () {
            return AddLastData({}).catch(res => {
                assert.equal(res.message, LastData_NOT_FOUND_CONTEXT)
            })
        })

        it('func LastData Add. Проверка на пустой объект', async function () {
            return AddLastData({},new User()).catch(res => {
    ...
    it('func LastData GetAll. Получение всех записей', async function () {
        return GetAllLastData({limit:10,offset:0},new User({})).then(res => {
            if (res.length) {
                res = res[0]
    ...
    it('func LastData Update. Редоктирование корректными значениями', async function () {
        ...
        return EditLastData(o,new User()).then(res => {
                                    ...
                                    assert.deepStrictEqual(res.data, {'dd' : 'sss'})
