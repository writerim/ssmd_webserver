...
const { 
    ...
    UserPlaceGetAllIncludesObject,
    UserPlaceGetAllIncludesObjectCount,
...
module.exports = {
    ...
    async GetAllUserPlace(params, flags , user_ctx) {
        if (!user_ctx || typeof user_ctx != 'object' || !(user_ctx instanceof UserCtx)) {
            throw new Error(NOT_FOUND_CONTEXT)
        }
        return UserPlaceGetAll(params, flags).then(res=>{
            let output = []
            res.forEach(r => {
    ...
    async GetAllByFilterUserPlace(filter, params , flags, user_ctx) {
        if (!user_ctx || typeof user_ctx != 'object' || !(user_ctx instanceof UserCtx)) {
            throw new Error(NOT_FOUND_CONTEXT)
        }
        return UserPlaceGetAllFilter(filter, params, flags).then(res=>{
            let output = []
            res.forEach(r => {
                output.push(new UserPlace(r.dataValues))
            })
    ...
    async GetAllSearchUserPlace(text, params , flags, user_ctx) {
        if (!user_ctx || typeof user_ctx != 'object' || !(user_ctx instanceof UserCtx)) {
            throw new Error(NOT_FOUND_CONTEXT)
        }
        return UserPlaceGetAllSearch(text, params, flags).then(res=>{
            let output = []
            res.forEach(r => {
                output.push(new UserPlace(r.dataValues))
            })
