...
const { 
    ...
    RoleGetAllFilter,
    RoleGetAllIncludesObject,
    RoleGetAllIncludesObjectCount,
...
module.exports = {
    ...
    async GetAllRole(params, flags , user_ctx) {
        if (!user_ctx || typeof user_ctx != 'object' || !(user_ctx instanceof UserCtx)) {
            throw new Error(NOT_FOUND_CONTEXT)
        }
        return RoleGetAll(params, flags).then(res=>{
            let output = []
            res.forEach(r => {
    ...
    async GetAllByFilterRole(filter, params , flags, user_ctx) {
        if (!user_ctx || typeof user_ctx != 'object' || !(user_ctx instanceof UserCtx)) {
            throw new Error(NOT_FOUND_CONTEXT)
        }
        return RoleGetAllFilter(filter, params, flags).then(res=>{
            let output = []
            res.forEach(r => {
                output.push(new Role(r.dataValues))
            })
    ...
    async GetAllSearchRole(text, params , flags, user_ctx) {
        if (!user_ctx || typeof user_ctx != 'object' || !(user_ctx instanceof UserCtx)) {
            throw new Error(NOT_FOUND_CONTEXT)
        }
        return RoleGetAllSearch(text, params, flags).then(res=>{
            let output = []
            res.forEach(r => {
                output.push(new Role(r.dataValues))
            })
